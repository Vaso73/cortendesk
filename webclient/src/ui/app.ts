...
// Static label tables are evaluated when the module loads, so initialize from
// the server-owned bootstrap before creating them. mount() repeats this after
// reading Window for clarity and for tests that install the payload later.
const injectedI18n = (globalThis as { __RD__?: { i18n?: Parameters<typeof configureI18n>[0] } }).__RD__?.i18n;
if (injectedI18n) configureI18n(injectedI18n);
...
export const PERMISSION_CONTROLS: Record<string, { id: string; title: string }> = {
  File: { id: 'rd-btn-files', title: t('permission.file') },
  Clipboard: { id: 'rd-btn-clip', title: t('permission.clipboard') },
  Keyboard: { id: 'rd-btn-cad', title: t('permission.keyboard') },
};
...
export class RdApp {
  ...
  mount(): void {
    ...
    if (this.cfg?.i18n) configureI18n(this.cfg.i18n);
    this.ensureDom();
    this.renderTopBar();
    this.renderDock();
    this.renderSide();
    ...
    this.el.toolbar.innerHTML = `
      <div class="rd-tb-brand">
        ...
        <span class="rd-tb-brandtext">
          ...
          <span class="rd-tb-sub">${h('app.subtitle')}</span>
        </span>
      </div>
      <div class="rd-tb-island">
        ...
        <button type="button" class="rd-ib rd-stream-only" id="rd-btn-monitors" title="${h('aria.selectMonitor')}" aria-label="${h('aria.selectMonitor')}" aria-haspopup="true" hidden>${iconHtml('monitor')}</button>
        <button type="button" class="rd-ib rd-stream-only" id="rd-btn-more" title="${h('app.more')}" aria-label="${h('app.more')}" aria-haspopup="true">${iconHtml('more')}</button>
        <span class="rd-island-sep rd-stream-only" aria-hidden="true"></span>
        <button type="button" class="rd-chip rd-stream-only" id="rd-btn-fit" aria-haspopup="true" title="${h('app.scale')}">
          <span id="rd-fit-label">${h('view.fit')}</span>${iconHtml('chevronDown')}
        </button>
        <button type="button" class="rd-chip rd-stream-only" id="rd-btn-viewonly" aria-pressed="false" title="${h('view.onlyHint')}">
          ${iconHtml('eye')}<span>${h('view.only')}</span>
        </button>
        <button type="button" class="rd-disconnect rd-stream-only" id="rd-btn-disconnect">${h('action.disconnect')}</button>
      </div>
  ...
  private toggleViewOnly(): void {
    ...
    this.toast(this.viewOnly ? t('view.onlyOn') : t('view.inputOn'));
  }
  ...
  private renderDock(): void {
    const db = (id: string, icon: IconName, label: string, title = label): string =>
      `<button type="button" class="rd-db" id="${id}" title="${escapeHtml(title)}" aria-label="${escapeHtml(title)}">` +
      `${iconHtml(icon)}<span>${escapeHtml(label)}</span></button>`;
    this.el.dock.innerHTML = `
      <div class="rd-dock-group" role="group" aria-label="${h('dock.keyboard')}">
        ${db('rd-lat-ctrl', 'keyboard', 'Ctrl', t('dock.ctrlHint'))}
        ${db('rd-lat-alt', 'keyboard', 'Alt', t('dock.altHint'))}
        ${db('rd-key-del', 'keyboard', t('dock.del'), t('dock.deleteHint'))}
        ${db('rd-btn-cad', 'keyboard', t('dock.keys'), t('permission.keyboard'))}
      </div>
      <span class="rd-dock-sep" aria-hidden="true"></span>
      <div class="rd-dock-group" role="group" aria-label="${h('dock.inputMode')}">
        ${db('rd-mode-pointer', 'pointer', t('dock.pointer'), t('dock.pointerHint'))}
        ${db('rd-mode-touch', 'touch', t('dock.touch'), t('dock.touchHint'))}
      </div>
      <span class="rd-dock-sep" aria-hidden="true"></span>
      <div class="rd-dock-group" role="group" aria-label="${h('terminal.send')}">
        ${db('rd-btn-type', 'typeText', t('dock.type'), t('dock.typeHint'))}
        ${db('rd-btn-clip', 'clipboard', t('dock.clipboard'), t('permission.clipboard'))}
      </div>
      <span class="rd-dock-sep" aria-hidden="true"></span>
      <div class="rd-dock-group" role="group" aria-label="${h('dock.panels')}">
        ${db('rd-btn-files', 'folderTransfer', t('dock.fileTransfer'))}
        ${db('rd-btn-chat', 'chat', t('panel.chat'))}
        ${db('rd-btn-session', 'info', t('dock.session'), t('dock.sessionDetails'))}
      </div>`;
    ...
    q<HTMLButtonElement>(d, '#rd-key-del').addEventListener('click', () => {
      this.pressControl(ControlKey.Delete, t('toast.deleteSent'));
    });
  ...
  private renderSide(): void {
    const tab = (id: SideTab, icon: IconName, label: string): string =>
      ...
      `${iconHtml(icon)}<span>${escapeHtml(label)}</span><i class="rd-badge" hidden></i></button>`;
    this.el.side.innerHTML = `
      <header class="rd-side-head">
        <div class="rd-side-tabs" role="tablist">
          ${tab('files', 'folderTransfer', t('panel.files'))}
          ${tab('chat', 'chat', t('panel.chat'))}
          ${tab('details', 'info', t('panel.details'))}
        </div>
        <button type="button" class="rd-ib" id="rd-side-close" title="${h('aria.closePanel')}" aria-label="${h('aria.closePanel')}">${iconHtml('close')}</button>
      </header>
      <div class="rd-side-body">
        ...
        <section class="rd-pane rd-pane-chat" data-pane="chat" hidden>
          <div class="rd-voice-call">
            <button type="button" class="rd-voice-call-btn" id="rd-voice-call" aria-describedby="rd-voice-call-status">${h('chat.voiceCall')}</button>
            <span id="rd-voice-call-status" role="status" aria-live="polite">${h('chat.off')}</span>
          </div>
          <div class="rd-chat-list" id="rd-chat-list"></div>
          <form class="rd-chat-compose" id="rd-chat-form">
            <input type="text" id="rd-chat-input" autocomplete="off" placeholder="${h('chat.placeholder')}" maxlength="2000">
            <button type="submit" class="rd-ib rd-chat-send" title="${h('terminal.send')}" aria-label="${h('chat.sendAria')}">${iconHtml('send')}</button>
          </form>
        </section>
        <section class="rd-pane rd-pane-details" data-pane="details" hidden>
          <dl class="rd-stats-body">
            <div class="rd-stat-row"><dt>${h('details.device')}</dt><dd id="rd-stat-device">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.user')}</dt><dd id="rd-stat-user">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.platform')}</dt><dd id="rd-stat-platform">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.peerVersion')}</dt><dd id="rd-stat-version">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.codec')}</dt><dd id="rd-stat-codec">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.resolution')}</dt><dd id="rd-stat-res">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.fps')}</dt><dd id="rd-stat-fps">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.bitrate')}</dt><dd id="rd-stat-bitrate">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.framesDropped')}</dt><dd id="rd-stat-dropped">—</dd></div>
            <div class="rd-stat-row"><dt>${h('details.duration')}</dt><dd id="rd-stat-duration">—</dd></div>
          </dl>
        </section>
      </div>`;
    this.el.edge.innerHTML = `
      <button type="button" class="rd-edge-btn" data-open="files" title="${h('permission.file')}" aria-label="${h('aria.openFileTransfer')}">${iconHtml('folderTransfer')}</button>
      <button type="button" class="rd-edge-btn" data-open="chat" title="${h('panel.chat')}" aria-label="${h('aria.openChat')}">${iconHtml('chat')}<i class="rd-badge" hidden></i></button>
      <button type="button" class="rd-edge-btn" data-open="details" title="${h('dock.sessionDetails')}" aria-label="${h('aria.openSessionDetails')}">${iconHtml('info')}</button>`;
  ...
  private openSide(tabName: SideTab): void {
    if (this.state !== 'streaming') {
      this.toast(t('common.connectFirst'));
      return;
    }
    ...
    if (tabName === 'files') {
      if (this.permissions.File === false) {
        this.toast(t('file.notPermitted'));
        return;
      }
  ...
  private openTerminalPanel(): void {
    if (!remoteInputAllowed(this.viewOnly, 'terminal')) {
      this.toast(t('chat.remoteTerminalViewOnly'));
      return;
    }
  ...
  private async toggleVoiceCall(): Promise<void> {
    ...
    if (this.state !== 'streaming') {
      this.toast(t('chat.requireSession'));
      return;
    }
    if (this.permissions.Audio === false) {
      this.toast(t('chat.audioNotPermitted'));
      return;
    }
    if (!this.remoteAudioEnabled) {
      ...
      this.toast(t('chat.enableRemoteAudio'));
      return;
    }
    ...
    if (!prepared.ok) {
      ...
      this.renderVoiceCall();
      this.toast(prepared.message ?? t('chat.micUnavailable'));
      return;
    }
  ...
  private onVoiceCall(state: 'waiting'|'accepted'|'rejected'|'closed', detail?: string): void {
    if (state === 'accepted') {
      ...
      this.voiceCall?.stop();
      this.voiceCallState = 'idle';
      if (state === 'rejected') this.toast(t('chat.declined'));
  ...
  private renderChatList(): void {
    ...
    this.el.chatList.innerHTML = this.chatLog.length
      ? this.chatLog
          ...
          .join('')
      : `<div class="rd-chat-empty">${h('chat.empty')}</div>`;
  ...
  private openMonitorPop(): void {
    this.openPop(this.el.btnMonitors, (pop) => {
      ...
      const resolutionHtml = currentDisplay && resolutions.length
        ? `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('details.resolution')}</div>` + resolutions.map((resolution) =>
          ...
          (isVirtualDisplay ? `<button type="button" class="rd-mi" data-custom-resolution><span class="rd-mi-pad"></span><span class="rd-mi-label">${h('display.customResolution')}</span></button>` : '')
        : '';
      const virtualHtml = virtual
        ? `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('display.virtuals')}</div>` +
          (virtual.impl === 'rustdesk_idd'
            ? [1, 2, 3, 4].map((id) => this.menuItem(null, t('display.virtual', { number: id }), virtual.rustdeskIds.includes(id), `virtual:${id}`)).join('') +
              this.menuItem(null, t('display.unplugAll'), false, 'virtual:all')
            : this.menuItem(null, t('display.addVirtual'), false, 'virtual:add') +
              this.menuItem(null, t('display.removeVirtual'), false, 'virtual:remove') +
              this.menuItem(null, t('display.unplugAll'), false, 'virtual:all'))
        : '';
      pop.innerHTML =
        `<div class="rd-pop-title">${h('aria.selectMonitor')}</div>` +
        this.displays.map((display, index) => {
          const label = display.name?.trim() || t('display.monitor', { number: index + 1 });
          return (
            ...
            `<span class="rd-mon-res">${display.width}×${display.height}${display.online ? '' : ` · ${t('status.offline')}`}</span></span>` +
      ...
      for (const button of pop.querySelectorAll<HTMLButtonElement>('.rd-resolution')) {
        button.addEventListener('click', () => {
          ...
          this.toast(t('display.resolutionRequested'));
          this.closePop();
        });
      }
      ...
      for (const button of pop.querySelectorAll<HTMLButtonElement>('[data-action^="virtual:"]')) {
        button.addEventListener('click', () => {
          ...
          this.toast(t('display.virtualRequested'));
          this.closePop();
        });
      }
    });
  ...
  private openMorePop(anchor: HTMLElement): void {
    this.openPop(anchor, (pop) => {
      ...
      const securityHtml = security.length
        ? `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('more.remoteControls')}</div>`
      ...
      const tools =
        canTerminal || canCamera
          ? `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('more.tools')}</div>` +
            (canTerminal ? this.menuItem(null, t('terminal.title'), false, 'terminal') : '') +
            (canCamera ? this.menuItem(null, t('more.viewCamera'), false, 'camera') : '')
          : '';
      ...
      const codecHtml = this.codecSupport.map((codec) =>
        this.menuItem(null, codec === 'auto' ? t('more.autoCodec') : codec.toUpperCase(), this.preferredCodec === codec, `codec:${codec}`),
      ).join('');
      ...
      const mediaHtml =
        canAudio || canClipboard || canRecord
          ? `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('more.sessionMedia')}</div>` +
            (canAudio
              ? this.menuItem(null, this.audioStarted ? t('audio.ready') : t('audio.start'), this.audioStarted, 'audioResume') +
                this.menuItem(null, t('more.remoteAudio'), this.remoteAudioEnabled, 'audioToggle') +
                this.menuItem(null, t('more.mute'), this.audioMuted, 'audioMute') +
                `<label class="rd-media-volume"><span>${h('more.volume')}</span><input data-action="audioVolume" type="range" min="0" max="100" value="${volumePercent}"><output>${volumePercent}%</output></label>`
              : '') +
            (canClipboard
              ? this.menuItem(null, t('more.textClipboard'), this.clipboardEnabled, 'clipboardToggle') +
                (canSendClipboard ? this.menuItem(null, t('more.syncClipboard'), false, 'clipboardSync') : '')
              : '') +
            (canRecord
              ? this.menuItem(null, this.recording ? t('more.stopRecording') : t('more.startRecording'), this.recording, 'recording')
              : '')
          : '';
      pop.innerHTML =
        this.menuItem('refresh', t('more.refreshVideo'), false, 'refresh') +
        this.menuItem(fs ? 'fullscreenExit' : 'fullscreen', fs ? t('more.exitFullscreen') : t('more.fullscreen'), false, 'fullscreen') +
        `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('more.imageQuality')}</div>` +
        this.menuItem(null, t('more.best'), this.quality === QUALITY.best, 'quality:best') +
        this.menuItem(null, t('more.balanced'), this.quality === QUALITY.balanced, 'quality:balanced') +
        this.menuItem(null, t('more.speed'), this.quality === QUALITY.speed, 'quality:speed') +
        `<label class="rd-display-slider"><span>${h('more.customQuality')}</span><input data-action="customQuality" type="range" min="10" max="100" value="${this.customQuality}"><output>${this.customQuality}</output></label>` +
        `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('more.frameCodec')}</div>` +
        this.menuItem(null, t('more.adaptiveFps'), this.adaptiveFps, 'adaptiveFps') +
        `<label class="rd-display-slider"><span>${this.adaptiveFps ? h('more.maximumFps') : h('more.customFps')}</span><input data-action="customFps" type="range" min="5" max="120" step="5" value="${this.customFps}"><output>${this.customFps}</output></label>` +
        codecHtml +
        (canRemoteCursor
          ? `<div class="rd-pop-sep"></div><div class="rd-pop-title">${h('more.remoteCursor')}</div>` +
            this.menuItem(null, t('more.showCursor'), this.showRemoteCursor, 'showRemoteCursor') +
            this.menuItem(null, t('more.followCursor'), this.followRemoteCursor, 'followRemoteCursor') +
            (this.displays.length > 1 ? this.menuItem(null, t('more.followWindow'), this.followRemoteWindow, 'followRemoteWindow') : '') +
            this.menuItem(null, t('more.enlargeCursor'), this.cursorScale > 1, 'cursorScale')
          : '') +
        securityHtml +
        tools +
        mediaHtml;
      ...
      pop.querySelector<HTMLButtonElement>('[data-action="adaptiveFps"]')?.addEventListener('click', () => {
        ...
        this.toast(this.adaptiveFps ? t('more.adaptiveOn') : t('more.fixedFps'));
        this.closePop();
      });
      ...
      pop.querySelector<HTMLButtonElement>('[data-action="audioToggle"]')?.addEventListener('click', () => {
        ...
        this.toast(this.remoteAudioEnabled ? t('audio.enabled') : t('audio.disabled'));
        this.closePop();
      });
      pop.querySelector<HTMLButtonElement>('[data-action="audioMute"]')?.addEventListener('click', () => {
        ...
        this.toast(this.audioMuted ? t('audio.muted') : t('audio.unmuted'));
        this.closePop();
      });
      ...
      pop.querySelector<HTMLButtonElement>('[data-action="clipboardToggle"]')?.addEventListener('click', () => {
        ...
        this.toast(this.clipboardEnabled ? t('clipboard.enabled') : t('clipboard.disabled'));
        this.closePop();
      });
  ...
  private activateSecurityControl(id: string): void {
    ...
    if (id === 'elevation') {
      if (!window.confirm(t('security.elevationQuestion'))) return;
      this.post({ c: 'requestElevation' });
      this.toast(t('security.elevationRequested'), true);
      this.closePop();
      return;
    }
    if (id.startsWith('privacy:')) {
      ...
      if (on && !window.confirm(t('security.privacyQuestion'))) return;
      this.privacyModePending = true;
      this.activePrivacyImplKey = implKey;
      this.post({ c: 'privacyMode', implKey, on });
      this.toast(t(on ? 'security.enablingPrivacy' : 'security.disablingPrivacy'), true);
      this.closePop();
      return;
    }
    if (id === 'blockInput') {
      ...
      if (on && !window.confirm(t('security.blockQuestion'))) return;
      this.blockInputPending = true;
      this.post({ c: 'blockInput', on });
      this.toast(t(on ? 'security.blockingInput' : 'security.restoringInput'), true);
      this.closePop();
      return;
    }
    if (id === 'lockScreen') {
      const sent = this.post(buildLockScreenKeyCommand());
      this.toast(sent ? t('security.lockSent') : t('security.lockNotSent'));
      this.closePop();
      return;
    }
    if (id === 'lockAfterSessionEnd') {
      ...
      this.toast(
        this.lockAfterSessionEnd
          ? t('security.lockAfterOn')
          : t('security.lockAfterOff'),
      );
      this.closePop();
    }
  }
  ...
  private openFitPop(): void {
    this.openPop(this.el.btnFit, (pop) => {
      pop.innerHTML =
        this.menuItem(null, t('view.fit'), this.fitMode === 'fit') +
        this.menuItem(null, t('view.actual'), this.fitMode === 'actual');
      ...
      items[0]?.addEventListener('click', () => set('fit', t('view.fit')));
      items[1]?.addEventListener('click', () => set('actual', t('view.actual')));
    });
  }
  ...
  private openKeysPop(anchor: HTMLElement): void {
    this.openPop(
      ...
      (pop) => {
        pop.innerHTML =
          `<div class="rd-pop-title">${h('type.sendToRemote')}</div>` +
          this.menuItem('keyboard', 'Ctrl+Alt+Del') +
          this.menuItem('keyboard', t('keys.windows')) +
          this.menuItem('keyboard', t('keys.printScreen')) +
          this.menuItem('keyboard', t('keys.escape')) +
  ...
  private openTypePop(anchor: HTMLElement): void {
    this.openPop(
      ...
      (pop) => {
        ...
        pop.innerHTML = `
          <div class="rd-pop-title">${h('type.title')}</div>
          <textarea id="rd-type-text" rows="4" placeholder="${h('type.placeholder')}"></textarea>
          <div class="rd-pop-actions">
            <button type="button" class="rd-chip rd-chip-solid" id="rd-type-send">${iconHtml('send')}<span>${h('type.sendKeystrokes')}</span></button>
          </div>`;
        ...
        q<HTMLButtonElement>(pop, '#rd-type-send').addEventListener('click', () => {
          ...
          this.toast(t('chat.typed', { count: text.length }));
          this.closePop();
        });
      },
      true,
  ...
  private renderOverlay(): void {
    ...
    this.el.overlay.innerHTML = `
      <div class="rd-card">
        ...
        <div class="rd-tagline">${t('app.tagline')} ${overlayVersion(this.cfg)}</div>
        <div class="rd-divider" aria-hidden="true"></div>
        <p class="rd-help">${h('overlay.help')}</p>
        <div class="rd-target" id="rd-target" hidden>
          <span class="rd-target-ic">${device}</span>
          <span class="rd-target-cap">${h('overlay.clientId')}</span>
        ...
        <label class="rd-field" id="rd-field-id" hidden>
          <span class="rd-input">
            ...
            <input id="rd-peer-id" type="text" inputmode="numeric" autocomplete="off" spellcheck="false" placeholder="${h('common.deviceId')}">
          </span>
        </label>
        <div class="rd-connect-row">
          <span class="rd-input">
            <span class="rd-input-ic">${lock}</span>
            <input id="rd-password" type="password" autocomplete="new-password" placeholder="${h('common.enterPassword')}">
          </span>
          <button type="button" class="rd-go" id="rd-connect" aria-label="${h('action.connect')}">${arrow}</button>
        </div>
        <label class="rd-save">
          <input type="checkbox" id="rd-save-pw">
          <span>${h('overlay.savePasswordDevice')}</span>
        </label>
        ...
        <button type="button" class="rd-chip" id="rd-restart-cancel" hidden>${h('overlay.cancelReconnectShort')}</button>
      </div>`;
  ...
  private hydrateSavedPassword(peerId: string): void {
    ...
    this.el.passwordInput.placeholder = has ? t('overlay.savedPassword') : t('common.enterPassword');
  }
  ...
  private secureContextProblem(): string | null {
    ...
    if (typeof isSecureContext !== 'undefined' && !isSecureContext) {
      return t('browser.insecure');
    }
    return t('browser.noDecoder');
  }
  ...
  private onConnectClick(): void {
    ...
    if (!peerId) {
      this.setOverlayError(t('common.enterDeviceId'));
      return;
    }
    if (!this.cfg) {
      this.setOverlayError(t('common.missingConfig'));
      return;
    }
    const blocked = this.secureContextProblem();
    if (blocked) {
  ...
  private beginRemoteRestart(): void {
    if (this.state !== 'streaming' || !this.reconnectConfig) {
      this.toast(t('restart.unavailable'));
      return;
    }
    if (!window.confirm(t('restart.question'))) return;
    ...
    this.toast(t('restart.sent'), true);
  }
  ...
  private scheduleRestartReconnect(detail?: string): void {
    ...
    if (delay === null) {
      this.clearRestartFlow();
      this.setState('error', `${t('restart.didNotReturn')}${detail ? ` — ${detail}` : ''}`);
      return;
    }
    ...
    this.el.reconnectCancel.hidden = false;
    this.setOverlayStatusText(t('restart.waitRetry', { attempt: flow.attempt, seconds: Math.ceil(delay / 1000) }));
  ...
  private cancelRestartReconnect(): void {
    ...
    this.toast(t('restart.cancelled'));
  }
  ...
  private expireRestartFlow(flow: RestartFlow): void {
    ...
    if (reconnecting) {
      this.teardown(true);
      this.clearSecurityState();
      this.setState('error', t('restart.didNotReturn120'));
    } else if (sessionStillStreaming) {
      this.toast(t('restart.timeout'));
    } else {
      this.setState('error', t('restart.didNotReturn120'));
    }
  }
  ...
  private startSession(config: SessionConfig): void {
    ...
    worker.onerror = (e: ErrorEvent) => this.setState('error', e.message || t('file.workerFailed'));
  ...
  private onEvent(ev: UiWorkerEvent): void {
    switch (ev.t) {
        ...
        void navigator.clipboard
          ...
          .catch(() => {
            if (sessionEpoch === this.sessionEpoch) {
              this.toast(t('clipboard.received'));
            }
          });
        break;
      }
      ...
      case 'blockInput':
        ...
        break;
      case 'elevation':
        if (ev.state === 'succeeded') this.toast(t('security.elevationApproved'));
        else if (ev.state === 'failed') this.toast(t('security.elevationFailed', { detail: ev.detail ? `: ${ev.detail}` : '' }));
        else this.toast(t('security.elevationWaiting'), true);
        break;
      case 'credentials':
      ...
      case 'uac':
        if (ev.on) {
          this.toast(t('security.uacOpen'), true);
        } else {
          this.hideToast();
          this.toast(t('security.uacClosed'));
        }
        break;
      case 'msgbox': {
      ...
      case 'loginError':
        ...
        if (this.connectedWithSavedHash) {
          ...
          this.el.passwordInput.placeholder = t('common.enterPassword');
          this.connectedWithSavedHash = false;
        }
        this.showOverlay();
        this.setOverlayBusy(false);
        this.setOverlayError(ev.message || t('common.loginFailed'));
        this.el.passwordInput.focus();
  ...
  private setState(state: SessionState, detail?: string): void {
    ...
    switch (state) {
      ...
      case 'error':
        ...
        this.showOverlay();
        this.setOverlayBusy(false);
        this.setOverlayError(detail || t('common.connectionFailed'));
        break;
      case 'closed':
        this.teardown();
        this.clearSecurityState();
        ...
        this.showOverlay();
        this.setOverlayBusy(false);
        this.setOverlayStatusText(t('state.closed'));
        break;
      default:
        this.showOverlay();
        this.setOverlayBusy(true);
        this.setOverlayStatusText(stateLabel(state) + (detail ? ` — ${detail}` : '') + '…');
    }
  }
  ...
  private applyPrivacyModeState(state: number, details: string, implKey: string): void {
    ...
    if (failed) this.toast(t('security.privacyFailed', { detail: details ? `: ${details}` : '' }));
  ...
  private applyBlockInputState(state: number, details: string): void {
    ...
    if (failed) this.toast(t('security.inputFailed', { detail: details ? `: ${details}` : '' }));
    else this.refreshSecurityToast();
  }

  private refreshSecurityToast(): void {
    if (this.blockInputOn) this.toast(t('security.blocked'), true);
    else if (this.privacyModeOn) this.toast(t('security.privacyActive'), true);
    else this.hideToast();
  }
  ...
  private applyPermission(kind: string, enabled: boolean): void {
    ...
    if (kind === 'Restart' && !enabled && this.restartFlow) {
      const sessionStillStreaming = this.state === 'streaming';
      this.clearRestartFlow();
      if (sessionStillStreaming) this.toast(t('restart.denied'));
      else this.setState('error', t('restart.deniedStopped'));
    }
    ...
    if (!target) {
      if (kind === 'Audio' || kind === 'Recording') {
        const kindLabel = t(kind === 'Audio' ? 'permission.audio' : 'permission.recording');
        this.toast(t('permission.changed', { state: t(enabled ? 'permission.enabled' : 'permission.disabled'), kind: kindLabel }));
      }
    ...
    for (const id of ids) {
      ...
      el.disabled = !enabled;
      if (id === target.id) {
        el.title = enabled ? target.title : t('permission.notPermitted', { kind: target.title });
    ...
    this.toast(t('permission.changed', { state: t(enabled ? 'permission.enabled' : 'permission.disabled'), kind: target.title.toLowerCase() }));
  }
  ...
  private async resumeAudioFromUserGesture(): Promise<void> {
    ...
    if (!this.audioPlayback || this.permissions.Audio === false) {
      this.toast(t('audio.unavailable'));
      return;
    }
    ...
    this.toast(resumed ? t('audio.ready') : t('audio.browserBlocked'));
  }
  ...
  private showClipboardSyncOffer(): void {
    ...
    text.textContent = t('clipboard.syncQuestion');
    const sync = document.createElement('button');
    sync.type = 'button';
    sync.textContent = t('clipboard.syncNow');
    const dismiss = document.createElement('button');
    dismiss.type = 'button';
    dismiss.textContent = t('clipboard.notNow');
  ...
  private async toggleRecording(): Promise<void> {
    ...
    if (!this.canRecordSession()) {
      this.toast(t('record.notPermitted'));
      return;
    }
    const sessionEpoch = this.sessionEpoch;
  ...
  private onRecordingState(active: boolean, startedAtMs?: number): void {
    ...
    if (!active) {
      this.recorder = undefined;
      if (wasRecording) this.toast(t('record.saved'));
    } else {
      this.toast(t('record.local'), true);
    }
  }

  private async sendClipboard(): Promise<void> {
    if (!this.canSendClipboard()) {
      this.toast(t('clipboard.sessionDisabled'));
      return;
    }
    const sessionEpoch = this.sessionEpoch;
    ...
    if (text === null) {
      this.toast(t('clipboard.unavailable'));
      return;
    }
    this.post({ c: 'clipboardText', text });
    this.toast(t('clipboard.sent'));
  }
  ...
  private async toggleFullscreen(): Promise<void> {
    try {
      ...
      this.toast(t('fullscreen.unavailable'));
    }
  }
